...
- Run the script.
  ```
  $MMOJO_SERVER_SCRIPTS/520-Test-Cosmo-Build.sh X86_64 compatible 1 llama-server
  ```
</details>
...
- Run the script. We run with `.` so variables can be defined and exported.
  ```
  $MMOJO_SERVER_SCRIPTS/520-Test-Cosmo-Build.sh aarch64 compatible 1 llama-server
  ```
</details>
...
- Run the script:
  ```
  $MMOJO_SERVER_SCRIPTS/520-Test-Cosmo-APE.sh compatible find-ape 1 llama-server
  ```
</details>
