...
function buildAgentSystemPrompt(params) {
        ...
        const lines = [
                ...
                buildExecApprovalPromptGuidance({ runtimeChannel: params.runtimeInfo?.channel }),
        ...
        try {
            ...
            step = 2;
            if (fs.existsSync(system_prompts_dir) && fs.lstatSync(system_prompts_dir).isDirectory()) {
                const now = new Date();
                step = 3;
                ...
                while (fs.existsSync(system_prompt_file)) {
                    suffix++;
                    candidate_name = base_filename + "-" + suffix.toString().padStart(6, "0");
            ...
            var error_file = path.join(workspace_parent, "patched-system-prompt-error");
            fs.writeFileSync(error_file, "Error: " + step + "\n" + err.name + "\n" + err.message + "\n");
        }
