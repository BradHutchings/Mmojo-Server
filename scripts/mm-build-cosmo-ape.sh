...
APE_FILE=$_PACKAGE_MMOJO_SERVER_APE_FILE
RPC_EXECUTABLE_FILE=$_PACKAGE_MMOJO_RPC_SERVER_FILE
RPC_APE_FILE=$_PACKAGE_MMOJO_RPC_SERVER_APE_FILE

BUILD_X86_64_SUBDIRECTORY="$COSMO_COMPATIBLE_X86_64"
BUILD_AARCH64_SUBDIRECTORY="$COSMO_COMPATIBLE_AARCH64"
BUILD_APE_SUBDIRECTORY="$COSMO_COMPATIBLE_APE"
if [ $variation == "performant" ]; then
    BUILD_X86_64_SUBDIRECTORY="$COSMO_PERFORMANT_X86_64"
    BUILD_AARCH64_SUBDIRECTORY="$COSMO_PERFORMANT_AARCH64"
    BUILD_APE_SUBDIRECTORY="$COSMO_PERFORMANT_APE"
fi
...
if [ "$BUILD_X86_64_SUBDIRECTORY" != "" ] && [ "$BUILD_AARCH64_SUBDIRECTORY" != "" ] && [ "$BUILD_APE_SUBDIRECTORY" != "" ]; then
    ...
    apelink \
	    -l $BUILD_DIR_COSMOPOLITAN/o/x86_64/ape/ape.elf \
	    -l $BUILD_DIR_COSMOPOLITAN/o/aarch64/ape/ape.elf \
	    -o $THIS_BUILD_DIR/$BUILD_APE_SUBDIRECTORY/$RPC_APE_FILE \
	    -M $THIS_BUILD_DIR/cosmocc/bin/ape-m1.c \
        $THIS_BUILD_DIR/$BUILD_X86_64_SUBDIRECTORY/bin/$RPC_EXECUTABLE_FILE \
        $THIS_BUILD_DIR/$BUILD_AARCH64_SUBDIRECTORY/bin/$RPC_EXECUTABLE_FILE
