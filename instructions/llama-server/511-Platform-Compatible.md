...
- Run the script.
  ```
  $MMOJO_SERVER_SCRIPTS/510-Build-for-Platform.sh compatible "$GPUS_CHOICE" llama-server
  ```
...
- Run the script.
  ```
  $MMOJO_SERVER_SCRIPTS/510-Test-Server.sh compatible "$GPUS_CHOICE" 1 llama-server
  ```
...
- Run the script:
  ```
  $MMOJO_SERVER_SCRIPTS/510-Copy-Executables-to-Share.sh compatible $GPUS_CHOICE
  ```
</details>
...
. mm-gpus-choose.sh
$MMOJO_SERVER_SCRIPTS/510-Build-for-Platform.sh compatible "$GPUS_CHOICE" llama-server
$MMOJO_SERVER_SCRIPTS/510-Copy-Executables-to-Share.sh compatible "$GPUS_CHOICE" llama-server
$MMOJO_SERVER_SCRIPTS/510-Test-Server.sh compatible "$GPUS_CHOICE" 1 llama-server
```
