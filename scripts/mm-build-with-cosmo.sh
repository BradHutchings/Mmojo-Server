...
if [ -v CC ]; then
    ...
    $MMOJO_SED -i -e 's/PUBLIC OpenSSL::SSL OpenSSL::Crypto/PUBLIC libssl.a libcrypto.a/g' vendor/cpp-httplib/CMakeLists.txt
    # Delete the rejection test for OpenSSL.
    $MMOJO_SED -i -e '/#include <openssl\/opensslv.h>/d' vendor/cpp-httplib/CMakeLists.txt
    $MMOJO_SED -i -e '/error bad version/d' vendor/cpp-httplib/CMakeLists.txt
